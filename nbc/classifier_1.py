...
def createPartitions(f):
	...
	indexl = range(0, 22242)
	random.shuffle(indexl)
	lines = f.readlines()
	partition_idx = []
	partition = []
	# first 8 partitions are of size of 2224
	# last 2 partitions are of size of 3402
	for i in range(0, 8):
		partition_idx.append(indexl[i*2224 : (i+1)*2224])
	partition_idx.append(indexl[8*2224 : 9*2224+1])
	partition_idx.append(indexl[9*2224+1 : 22242])
...
def learnNBC(f_train):
	...
	prior_n = 0.0
	neg_prior_n=0.0
	cond_n = [0] * num_feature
	neg_cond_n = [0] * num_feature
	...
	cond_p = [0.0] * num_feature
	neg_cond_p = [0.0] * num_feature
...
def applyNBC(prior_p, neg_prior_p, cond_p, neg_cond_p, f_test):
	...
	for test in f_test:
		...
		predClass = ''
		p = 0.0
		if log_neg_prior - log_prior >= 700:
			p_p=0.0
		else:
